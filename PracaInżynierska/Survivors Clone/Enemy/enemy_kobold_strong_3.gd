...
@export var movement_speed = 19.0  # 1.5x exponential scaling (Tier 3)
@onready var sprite = $Sprite2D
@export var hp = 51  # 1.5x exponential scaling (34 * 1.5 = 51)
...
@export var knockback_strength = 30.0  # Even less knockback
@export var knockback_duration = 0.12   # Shorter knockback duration
...
func _ready():
	...
	if has_node("Hitbox"):
		var hitbox = $Hitbox
		hitbox.damage = 18  # 1.5x exponential scaling (12 * 1.5 = 18)
	...
	print("Kobold Strong 3 collision setup - layer: ", collision_layer, " mask: ", collision_mask)
	...
	print("Kobold Strong 3 targeting: ", "Player" if target == player else ("Obelisk" if target == obelisk else "None"))
...
func _physics_process(delta):
	...
	if target:
		...
		print("Kobold Strong 3 has no target!")
...
func drop_loot(death_position: Vector2):
	...
	print("Kobold Strong 3 died at position: ", death_position)
	...
	# 4% chance to drop health pickup
	if randf() < 0.04:
		...
		print("Health pickup dropped from Kobold Strong 3!")
	
	# Add more EXP
	var game_stats = get_node("/root/GameStats")
	if game_stats:
		game_stats.add_exp(40)  # Kobold Strong 3 gives 40 EXP
...
# Functions removed - using simple charge movement only