...
@export var movement_speed = 15.0  # 1.5x exponential scaling (Tier 5 - Final)
@onready var sprite = $Sprite2D
@export var hp = 115  # 1.5x exponential scaling (77 * 1.5 = 115.5)
...
# Knockback variables (strongest resistance to knockback)
@export var knockback_strength = 10.0  # Minimal knockback
@export var knockback_duration = 0.08   # Very short knockback duration
...
func _ready():
	...
	if has_node("Hitbox"):
		var hitbox = $Hitbox
		hitbox.damage = 40  # 1.5x exponential scaling (27 * 1.5 = 40.5, rounded to 40)
	...
	print("Kobold Strong 5 collision setup - layer: ", collision_layer, " mask: ", collision_mask)
	...
	print("Kobold Strong 5 targeting: ", "Player" if target == player else ("Obelisk" if target == obelisk else "None"))
...
func _physics_process(delta):
	...
	if target:
		...
		print("Kobold Strong 5 has no target!")
...
func drop_loot(death_position: Vector2):
	...
	print("Kobold Strong 5 died at position: ", death_position)
	...
	# 7% chance to drop health pickup (highest)
	if randf() < 0.07:
		...
		print("Health pickup dropped from Kobold Strong 5!")
	
	# Add most EXP
	var game_stats = get_node("/root/GameStats")
	if game_stats:
		game_stats.add_exp(75)  # Kobold Strong 5 gives 75 EXP

func apply_knockback(attacker_position: Vector2):
	# Calculate knockback direction (away from attacker)
	var knockback_direction = (global_position - attacker_position).normalized()
	
	# Apply knockback velocity (minimal for strongest kobold)
