...
@export var movement_speed = 17.0  # 1.5x exponential scaling (Tier 4)
@onready var sprite = $Sprite2D
@export var hp = 77  # 1.5x exponential scaling (51 * 1.5 = 76.5)
...
@export var knockback_strength = 20.0  # Even less knockback
@export var knockback_duration = 0.1   # Shorter knockback duration
...
func _ready():
	...
	if has_node("Hitbox"):
		var hitbox = $Hitbox
		hitbox.damage = 27  # 1.5x exponential scaling (18 * 1.5 = 27)
	...
	print("Kobold Strong 4 collision setup - layer: ", collision_layer, " mask: ", collision_mask)
	...
	print("Kobold Strong 4 targeting: ", "Player" if target == player else ("Obelisk" if target == obelisk else "None"))
...
func _physics_process(delta):
	...
	if target:
		...
		print("Kobold Strong 4 has no target!")
...
func drop_loot(death_position: Vector2):
	...
	print("Kobold Strong 4 died at position: ", death_position)
	...
	# 5% chance to drop health pickup
	if randf() < 0.05:
		...
		print("Health pickup dropped from Kobold Strong 4!")
	
	# Add more EXP
	var game_stats = get_node("/root/GameStats")
	if game_stats:
		game_stats.add_exp(55)  # Kobold Strong 4 gives 55 EXP
