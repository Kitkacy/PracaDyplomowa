...
@export var knockback_strength = 50.0  # Less knockback than weak kobold
@export var knockback_duration = 0.2   # Shorter knockback duration
...
func _physics_process(delta):
	...
	if target:
		...
		print("Strong Kobold has no target!")
...
func drop_loot(death_position: Vector2):
	...
	print("Strong Kobold died at position: ", death_position)
	...
	# 2% chance to drop health pickup (higher than weak kobold)
	if randf() < 0.02:  # 2% chance
		...
		print("Health pickup dropped from Strong Kobold!")
	
	# Add more EXP than weak kobold
	var game_stats = get_node("/root/GameStats")
	if game_stats:
		game_stats.add_exp(25)  # Strong Kobolds give 25 EXP
